...
chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
  ...
  if (request.action === 'SEND_VERIFY_FLAG') {
    ...
    const { bookingId, agentEmail, confirmed, skipped, retroactive, imageBase64, mimeType, verifiedAt, workerUrl, vendor, product, vendorId, tourId } = request;
    (async () => {
      try {
        const controller = new AbortController();
        ...
        const res = await fetch(`${workerUrl.replace(/\/$/, '')}/confirm-flag`, {
          ...
          signal: controller.signal,
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ bookingId, agentEmail, confirmed, skipped, retroactive, imageBase64, mimeType, verifiedAt, vendor, product, vendorId, tourId }),
        });
        const data = await res.json().catch(() => ({}));
        if (res.ok && data.ok) {
